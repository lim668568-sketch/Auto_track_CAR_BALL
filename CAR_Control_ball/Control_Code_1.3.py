...
def sending_data(x_output, h_output, length_data):
    ...
    x_output = int(constrain(x_output, -127, 127))
    h_output = int(constrain(h_output, -127, 127))
    length_data = int(constrain(length_data, 0, 255))  # 距离限制在0-255范围
    ...
    if Key == 1:  # 停止模式
        ...
        if blobs and length > 0:  # 确保有目标且已设置长度
            ...
            if max_blob:
                ...
                # PID计算
                x_error = max_blob[5] - img.width() / 2  # 水平偏差
                ...
                sending_data(x_output, h_output,0)
                print(f"x_error: {x_error}, h_output: {h_output}")
        else:
            # 未识别到目标或未设置长度时停止
            #sending_data(0, 0)
            # 可选：添加原地旋转寻找目标的逻辑
             sending_data(18, -18,0)  # 小幅度旋转
